window.onload = function () {
  ...
  // if(getCookie("key")){
  //     key = getCookie("key"); // 从 cookie 中获取 key
  // }

  if (localStorage.getItem("key")) {
    key = localStorage.getItem("key"); // 从 localStorage 中获取 key
  }

  document.body.style.background = colorArr[key]; // 设置背景颜色
  ...
  document.querySelector(".changeSkin").onclick = function () {
    ...
    //     "Max-Age":3600*24
    // });
    localStorage.setItem("key", key); // 存储数据
